...
<script>
    $(document).ready(function(){
        var table = $('#example1').DataTable( { 
             "processing": true,
             "serverSide": true,
             "ajax": "pembelian-data.php?cabang=<?= $sessionCabang; ?>",
             "columnDefs": 
             [
              {
                "targets": 4,
                ...
                "targets": -1,
                  ...
                  `<center class="orderan-online-button">
                      ...
                      <?php if ( $levelLogin !== "admin" ) { ?>
                        <button class='btn btn-warning tblEdit' title="Retur">
        ...
        $('#example1 tbody').on( 'click', '.tblEdit', function () {
            ...
            if ( link === true ) {
                window.location.href = "pembelian-edit?no="+ data0;
            }
        });
        ...
        $('#example1 tbody').on( 'click', '.tblPrint', function () {
            var data = table.row( $(this).parents('tr')).data();
            var data0 = data[0];
            window.open('nota-cetak-pembelian?no='+ data0, '_blank');
        });
        ...
        $('#example1 tbody').on( 'click', '.tblDelete', function () {
            ...
            if ( link === true ) {
                window.location.href = "pembelian-delete-invoice?id="+ data1 + "&page=pembelian";
            }
        });
